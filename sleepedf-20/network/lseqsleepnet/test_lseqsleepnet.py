...
if (not eog_active and not emg_active):
    ...
    test_gen_wrapper.set_eeg_normalization_params(
    ...
    test_gen_wrapper.set_eeg_normalization_params(
        train_gen_wrapper.eeg_meanX, train_gen_wrapper.eeg_stdX)
    test_gen_wrapper.set_eog_normalization_params(
    ...
    test_gen_wrapper.set_eog_normalization_params(
        train_gen_wrapper.eog_meanX, train_gen_wrapper.eog_stdX)
    test_gen_wrapper.set_emg_normalization_params(
        train_gen_wrapper.emg_meanX, train_gen_wrapper.emg_stdX)
    nchannel = 3

# Must call next_fold() first so gen is populated
...
# Compute audio normalization from training set, apply to test set
# Must happen AFTER next_fold() so gen is populated
print("Computing audio normalization params...")
train_audio = train_gen_wrapper.gen.X_audio
mean_audio = np.mean(train_audio, axis=0)
std_audio = np.std(train_audio, axis=0)
test_gen_wrapper.gen.normalize_audio(mean_audio, std_audio)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def _reshape_batch(x_batch, audio_batch, y_batch):
            x_shape = x_batch.shape
            y_shape = y_batch.shape
            a_shape = audio_batch.shape
            ...
            for s in range(config.nsubseq):
                ...
                a[:, s] = audio_batch[:, s * config.sub_seq_len:(s + 1) * config.sub_seq_len]
            return x, a, y
        ...
        def dev_step(x_batch, audio_batch, y_batch):
            x, a, y = _reshape_batch(x_batch, audio_batch, y_batch)
            ...
            output_loss, total_loss, yhat, score = sess.run(
                [net.output_loss, net.loss, net.prediction, net.score],
                feed_dict)
        ...
        def _evaluate(gen):
            ...
            score = np.zeros([len(gen.data_index),
                              config.sub_seq_len * config.nsubseq,
                              config.nclass])
            factor = 20 * 4
            ...
            while test_step < num_batch_per_epoch:
                x_batch, audio_batch, y_batch, label_batch_ = gen.next_batch(
                    factor * config.batch_size)
                output_loss_, total_loss_, yhat_, score_ = dev_step(
                    x_batch, audio_batch, y_batch)
            ...
            if gen.pointer < len(gen.data_index):
                actual_len, x_batch, audio_batch, y_batch, label_batch_ = gen.rest_batch(
                    factor * config.batch_size)
                output_loss_, total_loss_, yhat_, score_ = dev_step(
                    x_batch, audio_batch, y_batch)
                output_loss += output_loss_
            ...
            yhat = yhat + 1  # labels start from 1
            return yhat, score, output_loss, total_loss

        def evaluate(gen_wrapper):
            N = int(np.sum(gen_wrapper.file_sizes) -
                    (config.sub_seq_len * config.nsubseq - 1) * len(gen_wrapper.file_sizes))
            ...
            for n in range(config.sub_seq_len * config.nsubseq):
                y[count:count + len(gen_wrapper.gen.data_index), n] = \
                    gen_wrapper.gen.label[
                        gen_wrapper.gen.data_index -
                        (config.sub_seq_len * config.nsubseq - 1) + n]
        ...
        test_acc, test_yhat, test_score, test_output_loss, test_total_loss = \
